...
namespace objl
{
	...
	private:
			...
			std::string icurline)
		{
            ...
            vector<uint64_t> pindixes(sface.size()); // positional indixes
            ...
            for (int i = 0; i < pindixes.size(); ++i)
            {
                for (int j = 0; j < pindixes.size(); ++j)
                {
                    if (i != j)
                    {
                        Adjacency[pindixes[i]].insert(pindixes[j]);
                    }
                }
            }
		}
