    ...
    def parse_events(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == 114: # r for recording
                    self.world.camera_manager.toggle_recording()
                elif event.key == 110: # n for next
                    self.world.camera_manager.next_sensor()
                elif event.key == 99: # toggle camera
...
class CameraManager(object):
    ...
    def next_sensor(self):
        ...
        print('Current sensor: ' + self.sensors[self.index][2])
...
class Lidar(object):
    ...
    @staticmethod
    def _save_lidar(weak_self, point_cloud):
        self = weak_self()
        if not self:
            return
        
        if self.recording:       
            point_cloud.save_to_disk('_out/lidar/%.6d.ply' % point_cloud.frame)
